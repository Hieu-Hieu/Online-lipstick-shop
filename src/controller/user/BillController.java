		...
		try {
			...
			if (check > 0) {
				...
				for (BillDetail bd : listBD) {
					...
					bang += "<td>" + BigDecimal.valueOf((bd.getProduct().getPrice() * bd.getQuantity())).toPlainString()
							+ "</td>";
					bang += "</tr>";

					tongbill += bd.getProduct().getPrice() * bd.getQuantity();
					// DecimalFormat decimalFormat = new DecimalFormat( Double.toString(tongbill));
					// System.out.println("Tong bill:
					// "+bd.getProduct().getPrice()+"X"+bd.getQuantity()+" "+decimalFormat);
				}
				bang += " </table>";
				...
				body += "<br> Mã đơn hàng: " + b.getBillID() + " tổng hóa đơn: "
						+ BigDecimal.valueOf((tongbill)).toPlainString() + "đ vào lúc: " + b.getDate();
