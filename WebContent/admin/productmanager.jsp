					...
					<div class="row">
						<div class="col-md-2">
							<form class="navbar-form"
								action="${pageContext.request.contextPath }/admin/product/list?command=search&currentPage=1"
								method="post">
													...
													<c:when test="${!empty listProduct}">
														<c:forEach items="${listProduct}" var="p">
															...
															<tr>
																...
																<td><img
																	src="<c:url value = "${p.getImgFirst() }"/>" alt=""
																	width="30px" height="30px"> <img
																	src="<c:url value = "${p.getImgLast() }"/>" alt=""
																	width="30px" height="30px"></td>
																<td>${p.getName()}</td>
																<td>Son thỏi</td>
																<td><fmt:formatNumber var="price" type="number "
																		pattern="###,###,###" value="${p.getPrice()}" />
																	${price}<span>VNĐ</span></td>
																<td>${p.getQuantity() }</td>
																<td><a
																	href="${pageContext.request.contextPath }/admin/product/update?id=${p.getProductID()}">Sửa/</a>
																	<a onclick="return ConfirmDelete();"
																	href="${pageContext.request.contextPath }/admin/product/delete?id=${p.getProductID()}">Xóa</a>
																</td>
															</tr>
							...
							<ul class="pagination justify-content-center">
								<c:choose>
									<c:when test="${currentPage > 1}">
										<li class="page-item"><a class="page-link"
											href="${pageContext.request.contextPath }/admin/product/list?command=list&currentPage=${currentPage - 1}"
										...
										<c:choose>
											<c:when test="${currentPage == page }">
												<li class="page-item active"><a class="page-link"
													href="${pageContext.request.contextPath }/admin/product/list?command=list&currentPage=${page}">${page }</a>
												</li>
											</c:when>
											<c:otherwise>
												<li class="page-item"><a class="page-link"
								...
								<c:choose>
									<c:when test="${currentPage < totalPage }">
										<li class="page-item"><a class="page-link"
											href="${pageContext.request.contextPath }/admin/product/list?command=list&currentPage=${currentPage +1}">Next</a>
										</li>
									</c:when>
									<c:otherwise>
										<li class="page-item disabled"><a class="page-link"
						...
						<div class="col-md-6  align-self-sm-end">
							<div class="border__button">
								<a href="${pageContext.request.contextPath}/admin/product/add"><button
										class="border__button--button">Thêm mới</button></a>
							</div>
						</div>
					</div>
				</div>
